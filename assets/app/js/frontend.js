			...
			'complete': function(){
				...
				$.ajax({
					...
					complete: function(){
						...
						$.ajax({
							type: "GET",					
							url: frontReg.access.urlApi + "/v1/frontendConfigurationAPI/frontendConfigurationByUser/" + $("input#username").val(),
							dataType: 'json',
							contentType: 'application/json',															
							headers : {
							...
							success: function(response) {						
								if ( response.length > 0 ){		
									...
									var menuJson = response[0].frontendConfiguration.menuJson || '';
									var mainJson = response[0].frontendConfiguration.mainJson || '';
